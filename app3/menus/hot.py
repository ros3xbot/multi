...
from app3.client.engsel import get_package_details, get_family
from app3.menus.package import show_package_details
from app3.menus.util import clear_screen, pause, print_panel, format_quota_byte, display_html, simple_number, get_rupiah, live_loading
from app3.client.purchase.ewallet import show_multipayment
from app3.client.purchase.qris import show_qris_payment
from app3.client.purchase.balance import settlement_balance
from app3.util import merge_hot1, merge_hot2
...
def show_hot_menu():
    ...
    while in_hot_menu:
        ...
        if not hot_packages:
            print_panel("⚠️ Ups", "Nggak ada data HOT bro 🚨")
            pause()
            return
        ...
        if choice.isdigit() and 1 <= int(choice) <= len(hot_packages):
            ...
            if not family_data:
                print_panel("⚠️ Ups", f"Gagal ngambil data family {family_code} bro 🤯")
                pause()
                continue
...
def show_hot_menu2():
    ...
    while in_hot_menu2:
        ...
        if not hot_packages:
            print_panel("⚠️ Ups", "Nggak ada data HOT2 bro 🚨")
            pause()
            return None
        ...
        if choice.isdigit() and 1 <= int(choice) <= len(hot_packages):
            ...
            while in_payment_menu:
                ...
                method_table.add_row("00", f"[{theme['text_sub']}]Balik ke menu sebelumnya ✌️[/]")
