...
from app2.menus.util import format_quota_byte, live_loading, print_panel
...
def get_family_data(
    ...
    with live_loading("Mengambil data family...", get_theme()):
...
def validate_msisdn(
    ...
    with live_loading(f"Memvalidasi nomor {msisdn}...", get_theme()):
        res = send_api_request(api_key, path, raw_payload, tokens["id_token"], "POST")
...
def change_member(
    ...
    with live_loading(f"Mengubah slot {slot_id} ke {new_msisdn}...", get_theme()):
        res = send_api_request(api_key, path, raw_payload, tokens["id_token"], "POST")
...
def remove_member(
    ...
    with live_loading(f"Menghapus member {family_member_id} dari family...", get_theme()):
        res = send_api_request(api_key, path, raw_payload, tokens["id_token"], "POST")
...
def set_quota_limit(
    ...
    with live_loading(f"Mengatur limit kuota untuk {family_member_id} ke {formatted_new_allocation}...", get_theme()):
        res = send_api_request(api_key, path, raw_payload, tokens["id_token"], "POST")
