...
from app2.menus.util import clear_screen, pause, print_panel, live_loading, simple_number
...
def show_family_grup_menu(return_package_detail: bool = False):
    while True:
        ...
        console.print(Panel(
            Align.center("Daftar Family Code", vertical="middle"),
        ...
        if semua_kode:
            ...
            console.print(Panel(
                "[italic]Belum ada family code tersimpan.[/italic]",
        ...
        if aksi == "t":
            ...
            print_panel("Berhasil" if success else "Kesalahan",
                        "Family code baru berhasil ditambahkan." if success else "Gagal menambah, family code sudah ada.")
            pause()

        elif aksi == "h":
            if not semua_kode:
                print_panel("Informasi", "Tidak ada kode untuk dihapus.")
                pause()
                continue
            idx = console.input("Nomor kode yang ingin dihapus: ").strip()
            if idx.isdigit() and 1 <= int(idx) <= len(semua_kode):
                ...
                if konfirmasi == "y":
                    ...
                    print_panel("Berhasil" if removed else "Kesalahan",
                                f"Family code {removed} berhasil dihapus." if removed else "Gagal menghapus.")
                else:
                    print_panel("Informasi", "Penghapusan dibatalkan.")
            else:
                print_panel("Kesalahan", "Nomor kode tidak valid.")
            pause()

        elif aksi == "e":
            if not semua_kode:
                print_panel("Informasi", "Tidak ada kode untuk diedit.")
                pause()
                continue
            idx = console.input("Nomor kode yang ingin diubah namanya: ").strip()
            if idx.isdigit() and 1 <= int(idx) <= len(semua_kode):
                new_name = console.input("Masukkan nama baru: ").strip()
                success = edit_family_name(int(idx) - 1, new_name)
                print_panel("Berhasil" if success else "Kesalahan",
                            "Nama family code berhasil diganti." if success else "Gagal mengganti nama.")
            else:
                print_panel("Kesalahan", "Nomor kode tidak valid.")
            pause()
            ...
            if selected:
                try:
                    ...
                    print_panel("Kesalahan", f"Gagal menampilkan paket: {e}")
            else:
                print_panel("Kesalahan", "Nomor kode tidak valid.")
            pause()
