...
from app2.menus.util import pause, clear_screen, format_quota_byte, print_panel, simple_number, nav_range
from app2.client.famplan import get_family_data, change_member, remove_member, set_quota_limit, validate_msisdn
...
def show_family_info(api_key: str, tokens: dict):
    ...
    while in_family_menu:
        ...
        if not res.get("data"):
            print_panel("Kesalahan", "Gagal mengambil data family.")
            pause()
            return
        ...
        if plan_type == "":
            print_panel("Informasi", "Anda bukan organizer family plan.")
            pause()
            return
        ...
        nav.add_row("limit <No> <MB>", "Atur Limit Kuota")
        nav.add_row("del <No>", "Hapus Member")
        ...
        if choice == "1":
            ...
            try:
                ...
                if res_change.get("status") == "SUCCESS":
                    print_panel("Berhasil", "Member berhasil diganti.")
                else:
                    print_panel("Kesalahan", res_change.get("message","Terjadi kesalahan."))
                console.print(json.dumps(res_change, indent=4))
            except ValueError:
                print_panel("Kesalahan", "Input slot tidak valid.")
            pause()
            ...
            try:
                ...
                if member.get("msisdn") == "":
                    print_panel("Informasi", "Slot kosong.")
                    pause()
                    continue
                ...
                if res_del.get("status") == "SUCCESS":
                    print_panel("Berhasil", "Member berhasil dihapus.")
                else:
                    print_panel("Kesalahan", res_del.get("message","Terjadi kesalahan."))
            ...
            try:
                ...
                if res_limit.get("status") == "SUCCESS":
                    print_panel("Berhasil", "Limit kuota berhasil diatur.")
                else:
                    print_panel("Kesalahan", res_limit.get("message","Terjadi kesalahan."))
                console.print(json.dumps(res_limit, indent=4))
            except ValueError:
                print_panel("Kesalahan", "Input quota tidak valid.")
            pause()
            ...
            print_panel("Informasi", "Perintah tidak dikenali.")
            pause()
